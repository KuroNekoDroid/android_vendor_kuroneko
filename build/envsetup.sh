...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/KNXD-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.kuroneko.device | grep -q "$KURONEKO_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $KURONEKO_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function lineageremote()
{
    ...
    local KURONEKO_USER=$(git config --get review.review.lineageos.org.username)
    if [ -z "$KURONEKO_USER" ]
    then
        git remote add lineage ssh://review.lineageos.org:29418/$PFX$PROJECT
    else
        git remote add lineage ssh://$KURONEKO_USER@review.lineageos.org:29418/$PFX$PROJECT
    fi
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.kuroneko.device | grep -q "$KURONEKO_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.kuroneko.device | grep -q "$KURONEKO_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/kuroneko/build/tools/repopick.py $@
}
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $KURONEKO_FIXUP_COMMON_OUT ]; then
